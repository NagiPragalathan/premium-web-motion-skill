...
`.gitignore` excludes three things that must never be published — `corpus/`,
`motionsites_free_prompts.json`, and the local-only source-case library inside the skill. Verify
before the first push:

```bash
git status --porcelain | Select-String "corpus|source-cases"    # must return nothing
```

### The source-case library is local-only

`references/source-cases/` holds the verbatim text of all 144 reference cases. It is installed on
this machine so the skill can read a real case before building, and it is **gitignored for the same
reason `corpus/` is** — the published skill is a distillation, not a redistribution.

The skill checks whether the directory exists and works either way: with it, it opens a matching
case per section; without it, it uses the distilled references and never mentions the difference.
So the published plugin is complete on its own, and nothing leaks.

Reinstall it after regenerating the corpus:

```powershell
robocopy corpus\prompts plugins\premium-web-motion-skill\skills\premium-web-motion-skill\references\source-cases *.md
